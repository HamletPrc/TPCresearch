...
int main(int argc, char * argv[]) {
    ...
    DefineGas("argon", 90, "Cisopropanol", 10);
	...
	// The situation of Ions
	        ...
	        if(zi2 > induct+driftdown+kapton*2+metal*4) { 	
		        sumIonsBackFlow += 1.;
	        } else if(zi2 >= induct+driftdown+kapton+metal*2 && zi2 <= induct+driftdown+kapton*2+metal*4) {
		        sumIonsTHGEMup += 1.;
	        } else if(zi2 >= induct && zi2<=induct+kapton+metal*2) {
		        sumIonsTHGEMdown += 1.;
	        } else {
		        sumIonsOther += 1.;
	        }
...
void ReadFiledMap(TString path)
{
    ...
    TString mfile = path+TString("/dat/Mesh900Double.mphtxt");
    TString dfile = path+TString("/dat/dielectrics_article.dat");
    TString ffile = path+TString("/dat/Field900Double.txt");
